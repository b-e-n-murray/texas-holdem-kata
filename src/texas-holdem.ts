...
//     && checkForStraight(combinedHand) === false
  ...
  if(checkForStraight(combinedHand) === false
    && checkForFlush(combinedHand) === true) {
    result.type = 'flush'
  }
...
//     && checkForStraight(combinedHand) === false
  ...
  if(Object.keys(occurrences).length === 2 
     && occurrences[Object.keys(occurrences)[0]] === 2
      && occurrences[Object.keys(occurrences)[1]] === 3) {
//     && checkForStraight(combinedHand) === false
    //if the occurrences obj has only one key/val pair, and its value is 3
    result.type = 'full house'
  }
  ...
  else if(Object.keys(occurrences).length === 2
          && occurrences[Object.keys(occurrences)[0]] === 4
          || occurrences[Object.keys(occurrences)[1]] === 4) {
...
//helper function that removes suits from a combined hand + river
  ...
  function checkForFlush(combinedHand: string[]): boolean { //should be using removeValues...
    // ['♥', '♠', '♠', '♥', '♠', '♠', '♦']
    // [h, s, d]
    const uniqueSuits: string[] = []
    for(const suit of combinedHand) {
